@extends('layouts.admin.app', ['body_class' => 'nav-md', 'title' => 'Edit Course Details'])
@section('content')
        ...
        <div class="row">
            <div class="col-8 offset-2">
                        ...
                        <form method="POST"
                            ...
                            <div class="form-group">
                                ...
                                @foreach ($categories as $cat)
                                        <option {{ (old('category_id', $course->category_id) == $cat->id) ? 'selected' : '' }} value="{{ $cat->id }}">{{$cat->title}} </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="amount" class="form-control" value="{{ old('amount', $course->amount) }}">
                            ...
                            <div class="form-group">
                                ...
                                <img class="w-100" src="{{ $course->getImage() }}" alt="">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="keyword" class="form-control" value="{{ old('keyword', $course->keyword) }}">
                                <x-input-error name='keyword' />
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1">Status</label>
                                <select name="status" class="form-control select2-single mb-3">
                                    <option {{ old('status', $course->status) == '1' ? 'selected' : '' }} value="1">
                                        Enabled
                                    </option>
                                    <option {{ old('status', $course->status) == '0' ? 'selected' : '' }} value="0">
                                        Disabled
                                    </option>
                                </select>
                            ...
                            <a href="{{ route('admin.course.index') }}" class="btn btn-info mb-0"> Cancel</a>
                ...
                <form id="deleteForm" method="POST"
                    action="{{ route('admin.course.destroy', [
                        'course' => $course,
                    ]) }}"
                    enctype="multipart/form-data">
                    @csrf
